...
class CIFAR10Expert():
    ...
    def generate_expert_labels(self, labels, binary=False):
        batch_size = len(labels)
        outs = np.zeros(len(labels),dtype=int)
...
class SyntheticExpertOverlap():
    ...
    def __call__(self, images = None, labels = None, labels_sparse=None):
        ...
        for i in range(0, batch_size):
            ...
            #debugging     
            # if type(labels_sparse) == int:
            #     if labels_sparse == 66: 
            #         print("labels[i] , outs[i]", labels[i],outs[i])
...
class GTSRBExpert:
    def __init__(self, classes_oracle, n_classes=43, p_in=1.0, p_out=0.1):
    ...
    def generate_expert_labels(self, labels, binary=False):
        B = len(labels)
        outs = np.zeros(B, dtype=int)
        for i, l in enumerate(labels):
            if l in self.classes_oracle:
                if random.random() < self.p_in:
                    outs[i] = 1 if binary else l
                else:
                    outs[i] = 0 if binary else random.randint(0, self.n_classes-1)
            else:
                if random.random() < self.p_out:
                    outs[i] = 1 if binary else l
                else:
                    outs[i] = 0 if binary else random.randint(0, self.n_classes-1)
        return outs
...
class HAM1000Expert:
    def __init__(self, classes_oracle, n_classes=7, p_in=1.0, p_out=0.1):
        if isinstance(classes_oracle, int):
            classes_oracle = [classes_oracle]
        self.classes_oracle = classes_oracle
        self.n_classes = n_classes
