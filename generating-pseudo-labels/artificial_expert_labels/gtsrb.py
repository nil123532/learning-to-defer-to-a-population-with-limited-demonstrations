...
def load_gtsrb_targets(root='./data'):
    """
    Load GTSRB labels directly using torchvision datasets.
...
def main(argv):    
    ...
    for l in labels:
        for strength in EX_STRENGTHS:
            ...
            for s in seeds:
                try:
                    in_file = f'expert_{s}_gtsrb_expert{strength}.{0}@{l}_predictions'
                    print(f'Preprocess artificial_expert_labels from file {in_file}')
                    p_strength = strength
                    file_location = f'gtsrb/L_{l}_p{p_strength}/'+in_file+'.json'
                ...
                train_gt_targets, test_gt_targets = load_gtsrb_targets(os.getcwd())
               
                file_location = f'gtsrb/L_{l}_p{p_strength}/transistion_matrix_{s}_expert{strength}.{s}@{l}_train.png'
                noise_transistion_matrix(predictions['train'], train_gt_targets, NUM_CLASSES,file_location)

                # #get transition matrix 
                file_location = f'gtsrb/L_{l}_p{p_strength}/transistion_matrix_{s}_expert{strength}.{s}@{l}_test.png'
                ...
                out_file = f'expert_{s}_gtsrb_expert{strength}.{s}@{l}_true_predictions'
                file_location = f'gtsrb/L_{l}_p{p_strength}/'+out_file+'.json'
            ...
            for train_experts in train_seeds:
                file_name = f'gtsrb/L_{l}_p{p_strength}/expert_{train_experts}_gtsrb_expert{strength}.{0}@{l}_predictions.json'
                with open(file_name, 'r') as f:
                    data = json.load(f)
                    print("length of train array", len(data['train']))
                    train_array.append(data['train'])

            for test_experts in test_seeds:
                file_name = f'gtsrb/L_{l}_p{p_strength}/expert_{test_experts}_gtsrb_expert{strength}.{0}@{l}_predictions.json'
                with open(file_name, 'r') as f:
                    data = json.load(f)
            ...
            np.save(f'gtsrb/L_{l}_p{p_strength}/train_array.npy', train_array)
            np.save(f'gtsrb/L_{l}_p{p_strength}/test_array.npy', test_array)
