...
def setup_default_logging(args, default_level=logging.INFO,
    ...
    if 'CIFAR' in args.dataset or 'NIH' in args.dataset or 'GTSRB' in args.dataset or 'ham10000' in args.dataset or 'FASHION' in args.dataset:
        focal_part = "focal" if args.focal else ""
        deeper_part = "deeper" if args.deeper else ""
        gamma = str(args.gamma) if args.focal else ""
        weighted = "weighted" if args.weighted else ""
        focal_match = "focal_match"  if args.match else ""
        with_attn = args.with_attn
    
        # Build the final directory name by skipping empty parts
        dir_name_parts = [
            f"ex{args.ex_strength}",
            f"x{args.n_labeled}",
            focal_part,
            gamma,
            weighted,
        ...
        # Filter out any empty strings to avoid double underscores
        dir_name = "_".join(filter(None, dir_name_parts))
        
        output_dir = os.path.join(args.dataset, args.exp_dir, dir_name)
    else:
