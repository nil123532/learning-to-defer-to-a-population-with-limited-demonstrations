...
from torchvision.datasets import CIFAR100, CIFAR10 , FashionMNIST , GTSRB
...
class FashionMNIST_Dataset(FashionMNIST):
    """Class representing the Fashion MNIST dataset

    """
    def __getitem__(self, index: int):
        """Get item from Fashion MNIST dataset
        :param index: Index
        :return: tuple
            - img: Images
            - target: Targets
...
def get_train_val_test_data(expert, binary=False, model='efficientnet_b1', valid=True, L=None, gt_targets=False,
    ...
    if dataset == 'cifar100':
        ...
        return get_cifar10_data(expert, binary, model, valid, L, gt_targets , seed)
    elif dataset == 'nih':
        ...
        return load_ham10000(model=model)
    elif dataset == 'fashion':
        return load_fashion_mnist(model=model)
...
def get_cifar10_data(expert, binary=False, model='efficientnet_b1', valid=True, L=None, gt_targets=False, seed=123):
    """Generate the train, validation and test set for the cifar100 dataset
    ...
    train_transform = tt.Compose([
        tt.Resize(img_size),
        tt.RandomAffine(degrees=2, translate=(0.02, 0.02), scale=(0.98, 1.02), shear=2),
        tt.RandomHorizontalFlip(),
        tt.ToTensor(),
    ...
    # load train, test and validation data
    train_data = CIFAR10_Dataset(download=True, root="./data", transform=train_transform)
    unlabeled_data = CIFAR10_Dataset(root="./data", transform=train_transform)
    val_data = CIFAR10_Dataset(root="./data", transform=test_transform)
    test_data = CIFAR10_Dataset(root="./data", train=False, transform=test_transform)
    ...
    train_gt_data = train_data.targets
    test_gt_data = test_data.targets
...
def load_ham10000(root: str = 'data/HAM10000',model='ham10000'):
    """
    Loads pre-saved HAM10000 train/val/test .pt files and wraps them
    in a Dataset that returns (img, label, index).
    """
    ds_path = root.rstrip('/') + '/'
    train_data = torch.load(ds_path + 'train_data.pt')
    val_data   = torch.load(ds_path + 'validation_data.pt')
    test_data  = torch.load(ds_path + 'test_data.pt')

    # define your transforms here (e.g., ToTensor, Normalize, augmentations…)
    ...
    if model.startswith('vit'):
        ...
        transform_test.transforms.append(transforms.Resize((224, 224)))

    print(f"Train data size: {len(train_data['data'])}")
    print(f"Validation data size: {len(val_data['data'])}")
    print(f"Test data size: {len(test_data['data'])}")

    train_ds = HAM10000_Dataset(train_data['data'],
                                train_data['labels'],
                                transform=transform_train)
    val_ds   = HAM10000_Dataset(val_data['data'],
                                val_data['labels'],
                                transform=transform_test)
    test_ds  = HAM10000_Dataset(test_data['data'],
                                test_data['labels'],
                                transform=transform_test)
...
class HAM10000_Dataset(Dataset):
    """HAM10000 dataset that returns (img, target, index)."""
    def __init__(self, images: np.ndarray, targets: list, transform=None):
        """
        images: numpy array of shape (N, H, W, C) or torch.Tensor
        targets: list or array of length N
        transform: torchvision transform (expects a PIL.Image or torch.Tensor)
        """
        self.images = images
...
def load_fashion_mnist(model):
    ...
    train_transform = tt.Compose([
        ...
        tt.RandomHorizontalFlip(),
        tt.ToTensor(),
        tt.Lambda(lambda x: x.repeat(3, 1, 1)),  # Convert 1-channel to 3-channel
        tt.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])
    ...
    train_data = FashionMNIST_Dataset(root="./data", train=True, download = True , transform=train_transform)
    val_data = FashionMNIST_Dataset(root="./data", download=True ,transform=test_transform)
    test_data = FashionMNIST_Dataset(root="./data", train=False, download=True, transform=test_transform)
...
class GTSRB_Dataset(GTSRB):
    """Return (img, target, index) and expose .targets for easy splitting."""
    def __init__(self, root, split="train", transform=None, download=False):
        super().__init__(root=root, split=split,
                         transform=transform, download=download)
        # expose class labels for StratifiedShuffleSplit
        self.targets = [label for _, label in self._samples]

    def __getitem__(self, index):
        # GTSRB already gives RGB PIL images; just grab & return index
...
def load_gtsrb(
    ...
    Returns
    -------
    train_ds : Dataset  # full official training set
    ...
    train_tf = tt.Compose([
        tt.Resize(IMG_SIZE),
        tt.RandomAffine(5, translate=(0.05, 0.05), scale=(0.9, 1.1), shear=5),
        tt.RandomHorizontalFlip(),
        *_base,
    ])
    ...
    train_full = GTSRB_Dataset(root, split="train",
                               transform=train_tf, download=True)
    test_full  = GTSRB_Dataset(root, split="test",
                               transform=test_tf,  download=True)

    # outputs -----------------------------------------------------------------
    train_ds = train_full        # whole training set
    val_ds   = test_full         # whole test set as validation
    test_ds  = test_full         # whole test set as test
