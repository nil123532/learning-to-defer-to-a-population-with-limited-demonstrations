...
def get_train_loader(dataset, expert, batch_size, mu, n_iters_per_epoch, L, root='data', method='comatch', imsize=32,weighted=False):
    ...
    if dataset == 'CIFAR10' or dataset == 'CIFAR100':
        ...
        cntx_sampler = ContextSampler(data_x,label_x,transform_train,n_cntx_pts=43,device='cuda')
        ds_x = GTSRBDataset(
            data=data_x,
            labels=label_x,
            mode='train_x',
...
def get_val_loader(dataset, expert, batch_size, num_workers, pin_memory=True, root='data', imsize=32):
    ...
    if dataset == 'GTSRB':
        ...
        cntx_sampler = ContextSampler(data_x,labels_x,transform_test,original_indices=indices_x,n_cntx_pts=43,device='cuda')
        ds = GTSRBDataset(
            data=data_test,
            labels=labels_test,
            mode='test',
