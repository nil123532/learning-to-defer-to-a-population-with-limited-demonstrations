...
def main(argv):
    ...
    train_dir = get_train_dir(wkdir, args, args['name'])
    ...
    for epoch in range(start_epoch, 200):
        ...
        valid_acc = emb_model.get_validation_accuracy(return_acc=True)
        # get test accuracy
        test_acc = emb_model.get_test_accuracy(return_acc=True)
        print(f'loss: {loss}')
        # save logs to json
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('name','pretrained','scratch')
    flags.DEFINE_bool('focal_loss',False,'Whether to use focal loss.')
    flags.DEFINE_integer('gamma',2,'Focal loss gamma parameter')
    flags.DEFINE_bool('weighted_loss',False,'Whether to use weighted loss.')
    app.run(main)
