...
class ClassifierRejectorWithContextEmbedder(nn.Module):
    def __init__(self, base_model, num_classes, n_features, dim_hid=128, depth_embed=6, depth_rej=4, dim_class_embed=128,
                 with_attn=False, with_softmax=True, decouple=False,train_type='f'):
        ...
        self.embed_class = nn.Embedding(num_classes, dim_class_embed) # created a class embedding layer
    ...
    def forward(self, x, cntxt=None):
        ...
        #replace self.base_model_rej with embedding model
        ...
        #replace self.rejector with expertise model
    ...
    @torch.no_grad()                    
    def get_context_encoding(self,cntxt ) -> torch.Tensor:
        """
        Returns an embedding that summarizes each expert's context set.
        ...
        cntxt : AttrDict | None
            Must contain
                ...
                yc [E, Nc]
                mc [E, Nc]
            If None, a zero vector is returned (one dummy expert).

        Returns
        -------
        torch.Tensor
            Shape [E, H]   (H == self.dim_hid).  No batch dimension, because
            it is purely a summary of the context examples.
        """
        if cntxt is None:
            return torch.zeros(
                1, self.dim_hid,
        ...
        # ---------- 1. embed each context image ----------
        xc_flat = cntxt.xc.view((-1,) + cntxt.xc.shape[-3:])         # [E*Nc,3,32,32]
        if not self.decouple:
            with torch.no_grad():
                xc_embed = self.base_model_rej(xc_flat)              # [E*Nc,Dx]
            xc_embed = xc_embed.detach()
        else:
            xc_embed = self.base_model_rej(xc_flat)                  # [E*Nc,Dx]

        xc_embed = xc_embed.view(cntxt.xc.shape[:2] + (xc_embed.size(-1),))  # [E,Nc,Dx]

        # ---------- 2. embed labels & predictions ----------
        yc_embed = self.embed_class(cntxt.yc)                        # [E,Nc,H]
        mc_embed = self.embed_class(cntxt.mc)                        # [E,Nc,H]

        # concat → [E,Nc,Dx+2H]  → MLP/Attn
        joint = torch.cat([xc_embed, yc_embed, mc_embed], dim=-1)
        feat  = self.embed(joint)                                    # [E,Nc,H]

        if not self.with_attn:
            return feat.mean(dim=-2)                                 # [E,H]

        # ---------- 3. attention variant ----------
        # Use the average of image features as a query when no x is provided
        query = xc_embed.mean(dim=-2, keepdim=True)                  # [E,1,Dx]
        summary = self.attn(query, xc_embed, feat).squeeze(1)        # [E,H]
        return summary
